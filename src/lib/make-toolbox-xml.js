...
const extrarCodenowBlocks = `
<block type="argument_reporter_boolean"><field name="VALUE">is compiled?</field></block>
<block type="argument_reporter_boolean"><field name="VALUE">is rCodenow?</field></block>
`;
...
const makeToolboxXML = function (isInitialSetup, isStage = true, targetId, categoriesXML = [],
    ...
    // Always display rCodenow blocks as the first extension, if it exists,
    // and also add an "is compiled?" block to the top.
    let rCodenowXML = moveCategory('tw');
    if (rCodenowXML && !rCodenowXML.includes(extrarCodenowBlocks)) {
        rCodenowXML = rCodenowXML.replace('<block', `${extrarCodenowBlocks}<block`);
    }
